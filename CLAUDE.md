...
dotnet test sources/CaveOfWonders.Tests/CaveOfWonders.Tests.csproj --filter "FullyQualifiedName~ConvertCurrencyUseCase"
...
- There are two data-access adapters. The CLI (`Setup.cs`, `DependenciesSetup.Configure`) picks one at startup via the `DatabaseType` setting in `appsettings.json` (`Json` or `SQLite`) and registers the matching `Database`/`UnitOfWork`; **the CLI's checked-in `appsettings.json` currently sets `DatabaseType: SQLite`, so `Adapters.DataAccess.SQLite` is the CLI's production adapter.** The WebApi (`Program.cs`) does not read `DatabaseType` — it always registers `Database`/`UnitOfWork` from `DustInTheWind.CaveOfWonders.Adapters.DataAccess.Json`, so JSON remains the WebApi's production adapter:
  ...
  - **`Adapters.DataAccess.SQLite`** — EF Core-backed, also full support including gems (`CaveOfWondersDbContext`, `*Entity` classes). Uses a checked-in **compiled model** (`CompiledModels/`, generated by `dotnet ef dbcontext optimize`) instead of runtime reflection, since the CLI is a short-lived process. Migrations and the compiled model are independent artifacts that must both be regenerated by hand whenever an entity or `IEntityTypeConfiguration<T>` changes — EF does not detect drift, and a stale compiled model is a silent correctness bug (wrong/missing columns), not a build failure. See `docs/EFCore-CompiledModels.md` and `docs/EFCore-Migrations.md` for the regeneration steps and checklist.
  - `sources/CaveOfWonders.Tests.Integration/tests-config.json` lists which adapters each repository port is tested against (e.g. `IPotRepository` → Json/SQLite; `IGemRepository` → Json/SQLite only).
- JSON storage model types are prefixed with `J` (e.g., `JGem`, `JPot`, `JExchangeRate`) and live in `*Storage/` subdirectories.
- CLI commands use **ConsoleTools.Commando** for routing. Each command class in `Cli.Presentation` is auto-discovered via `RegisterCommandsFrom`. Commands are organized into area folders: `PotArea` (includes pot management plus `Gain`/`Wealth`), `FxArea`, `CpiArea`, `WageArea`. See `sources/CaveOfWonders.Cli.Presentation/CLAUDE.md` for the full command/view/view-model scaffolding convention.
...
`Database.cs` in `Adapters.DataAccess.Json` (namespace `...DataAccess.Json`, folder `sources/CaveOfWonders.Adapters.DataAccess.Json/`) is an in-memory working set. It loads data from disk on initialization and flushes via `IUnitOfWork.SaveChangesAsync()`. Use cases access all repositories through `IUnitOfWork`, which is the only constructor-injected data-access dependency they should take. Gems are lazy-loaded on first access (`LoadGemsAsync` is guarded by an `areGemsLoaded` flag); all other collections load eagerly on `Database.LoadAsync`.
...
- **`FindAsync(GemFilter, ...)`** — the preferred method for flexible cross-pot queries. `GemFilter` (in `Ports.DataAccess`) supports composable predicates: `PotId`, `StartDate`/`EndDate`, `Date`, `Month`, `IncludeCategories`/`ExcludeCategories` (lists — translated to `WHERE category IN (...)` / `NOT IN (...)` in SQL adapters), and `ExternalId`. Prefer this over adding new single-purpose methods.
